...
import pathlib
import sys
QACE_ROOT_DIR = "%s/../../src" % (pathlib.Path(__file__).parent)
GLIB_ROOT_DIR = pathlib.Path(__file__).parent.as_posix()
sys.path.append(pathlib.Path(QACE_ROOT_DIR).as_posix())
...
class Runner:
    ...
    def __init__(self, agent, train_env, test_env, domain_name, 
                 ...
                 qace_callback=None,
                 vd_transitions=None):
        self.qace_callback = qace_callback
        self.agent = agent
    ...
    def _initialize_variational_distance_transitions(self):
        ...
        # MAJOR HACK FOR QACE NEW DOMAINS: Add a counter
    ...
    def run(self):
        ...
        while True:
            ...
            if itr % ac.learning_interval[self.domain_name] == 0:
                ...
                # QACE change: Always run tests no matte what!
                if self.qace_callback \
...
def get_glib_runner_from_qace(seed, domain_name, curiosity_name, learning_name,
                              qace_callback, vd_transitions):

    assert qace_callback is not None
    ...
    runner = Runner(agent, train_env, test_env, domain_name, curiosity_name,
                    qace_callback=qace_callback, vd_transitions=vd_transitions)
