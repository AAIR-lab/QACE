...
class Literal:
    ...
    def ground(self, params_map, with_copy=True):
        ...
        for i in range(len(self.variables)):
            ...
            literal.update_variable(i, params_map[self.variables[i]])
...
class ProbabilisticEffect:
    ...
    def optimize(self, with_copy=True):
        ...
        common_literals = set()
        if len(probabilistic_effect.literals) > 1:
            common_literals |= set(probabilistic_effect.literals[0].literals)
            
        for i in range(1, len(probabilistic_effect.literals)):
