...
def check_args_for_qace(args):
    pass

def check_and_get_actual_base_dir(args):
    base_dir = "%s/%s" % (args.base_dir, args.gym_domain_name)
    base_dir = os.path.abspath(base_dir)
    assert os.path.isdir(base_dir) or not os.path.exists(base_dir)

    vd_filepath = VariationalDistance.get_default_filepath(
        base_dir, args.gym_domain_name.lower())
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(prog="QACE",
                                     description="QACE code (NeurIPS-23)")
    ...
    parser.add_argument("--gym-domain-name", required=True, type=str,
                        help="The gymd domain name to use")
    ...
    parser.add_argument("--qace", default=False, action="store_true",
                        help="Run QACE")

    parser.add_argument("--problem-idx", default=0, type=int,
                        help="The problem index to use for training QACE")

    parser.add_argument("--sampling-count", default=5, type=int,
                        help="The sampling count to use for QACE.")
    ...
    parser.add_argument("--count-sdm-samples", default=False,
                        action="store_true",
                        help="Set this flag to count SDM samples in QACE")
    ...
    if "PYTHONHASHSEED" not in os.environ:
        print("PYTHONHASHSEED=0 must be set while running QACE.")
        parser.print_usage()
        sys.exit(1)

    print("Using PYTHONHASHSEED:", os.environ["PYTHONHASHSEED"])

    if sum([args.vd, args.qace, args.glib]) != 1:
        print("Exactly  one of --vd, --qace, --glib must be passed.")
        parser.print_usage()
        sys.exit(1)

    check_args_for_vd(args)
    check_args_for_glib(args)
    check_args_for_qace(args)
    ...
    if args.vd and not args.dry_run:
        VariationalDistance.generate_bfs_samples(args.gym_domain_name,
        ...
        vd_transitions = VariationalDistance.load_transitions(vd_filepath)
        qace_runner = SAIAEvaluator(args.gym_domain_name, result_logger,
                                    vd_transitions, args.disable_evaluator)

        if not args.dry_run:
            qace_runner.run(args, data_dir=data_dir)
