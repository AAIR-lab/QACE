...
class SAIAEvaluator:
    ...
    def __init__(self, gym_domain_name, result_logger, vd_transitions,
        ...
        self.experiment_params = {
            ...
            "naming_map": config.NAMING_MAP[self.gym_domain_name],
            "args_func_map": config.ARGS_FUNC_MAP[self.gym_domain_name],
        }
        self.vd_transitions = vd_transitions
        self.disable_evaluator = disable_evaluator
        self.env = gym.make(
            "PDDLEnv{}-v0".format(self.gym_domain_name),
    ...
    def run(self, args, **kwargs):
        ...
        if args.gym_domain_name == "Cafeworld":
            agent = CafeworldAgent(args.gym_domain_name,
                                   big_problem=False,
                                   params=self.experiment_params,
                                   base_dir=data_dir)
        else:
            agent = PRPAgent(args.gym_domain_name, args.problem_idx, None,
                             self.experiment_params)
...
if __name__ == "__main__":
    ...
    qace_evaluator = QACEEvaluator(experiment["base_dir"],
                                   experiment["gym_domain_name"],
                                   experiment["naming_map"],
                                   experiment["args_func_map"])
    
    from model import Model
    ground_truth_model = Model(qace_evaluator.domain)
    
    qace_evaluator.qace_callback(evaluation_model=ground_truth_model,
                                 itr=0,
                                 num_pal_tuples=None,
