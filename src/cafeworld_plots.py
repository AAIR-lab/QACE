...
def get_data(filepath, xkey, ykey, alg, metric):
    ...
    for row in reader:
        ...
        x_value, qace_sample = get_interaction_adjustment(xkey,
                                                          alg,
                                                          x_value,
                                                          qace_sample)

        y_value, qace_sample = get_interaction_adjustment(ykey,
                                                          alg,
                                                          y_value,
                                                          qace_sample)
...
def get_filename(domain, alg):
    ...
    if alg == "QACE-R":
        suffix = "qace_randomized.csv"
    elif alg == "QACE-S":
        suffix = "qace_sequential.csv"
    elif alg == "GLIB_G1":
        suffix = "glib_g1.csv"
    elif alg == "GLIB_L2":
        suffix = "glib_l2.csv"
...
def plot_supplemental(results_dir, domains, domain_data, num_bins):
    ...
    PLOTTERS = [("QACE-R", "QACE-R (Ours)", "blue", "-"),]
    ...
    for column, domain in enumerate(domains):
        ...
        for ax, metric in zip(axes, metrics):
            for alg, label, color, linestyle in PLOTTERS:
                ...
                if marker_point != len(x) - 1 and alg == "QACE-R":
...
if __name__ == "__main__":
    ...
    for column, DOMAIN in enumerate(DOMAINS):
        ...
        for run_no in RUNS:
            ...
            qace_r_file = "%s/run%u/%s/qace_randomized_sdm_counted.csv" % (BASE_DIR, run_no,
                                                                           DOMAIN)
            ...
            for metric in metrics:
                data["QACE-R"][X_METRIC][run_no], data["QACE-R"][metric][run_no] = get_data(qace_r_file,
                                                                                            X_METRIC, metric, "QACE-R",
                                                                                            metric)
