...
class PRP:
    ...
    def generate_pddlgym_samples_using_policy(env, domain, problem, policy,
        ...
        if obs.literals == policy.G.nodes[PRPPolicy.INIT_NODE_IDX]["label"]:
            ...
            node_idx = [PRPPolicy.INIT_NODE_IDX]
        else:
        ...
        while execution_status \
            and h < H \
            and len(node_idx) == 1 \
            and node_idx[0] != PRPPolicy.GOAL_NODE_IDX:

            # Get the node idx.
            node_idx = node_idx.pop()

            # Get the action corresponding to it.
            actions = set()
            possible_successors = []                    
            edges = policy.G.edges(node_idx, data=True)
            for edge in edges:
