...
for domain in "Tireworld" "Explodingblocks" "First_responders" "Probabilistic_elevators"
do
    python3 src/main.py --base-dir $RESULTS_DIR --gym-domain-name $domain --qace --count-sdm-samples &> "$RESULTS_DIR"/"$domain"_qace_s_c.log &
    python3 src/main.py --base-dir $RESULTS_DIR --gym-domain-name $domain --randomize-pal --qace --count-sdm-samples &> "$RESULTS_DIR"/"$domain"_qace_r_c.log &
